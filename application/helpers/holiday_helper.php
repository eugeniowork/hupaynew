<?php 
    ...
    function holidayCutOffCount(){
        ...
        $has_holiday = 0;
        $select_qry = $CI->holiday_model->get_holiday();
        if(!empty($select_qry)){
    ...
    function getCutOffHoliday(){
        ...
        if(!empty($select_qry)){
            foreach ($select_qry as $value) {
                ...
                // if sakop
                if ($holiday_date >= $final_date_from && $holiday_date <= $final_date_to){
                    $finalData .= "<li><b>" .date_format(date_create($value->holiday_date. ", " . $year),"F d, Y") ."</b> - <span style='color:#317eac;'>" . $value->holiday_value . "</span> (<i>". date_format(date_create($value->holiday_date. ", " . $year),"l") . "</i>)</li>";
