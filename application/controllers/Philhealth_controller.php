...
class Philhealth_controller extends CI_Controller{
    function __construct(){
		parent::__construct();
        if(!$this->session->userdata('user')){
            redirect('login');
        }
        $this->load->model("philhealth_model", 'philhealth_model');
        $this->load->model("employee_model", 'employee_model');
        $this->load->helper('hupay_helper');
    }

    public function index(){
        $this->data['pageTitle'] = 'Philhealth Contribution Table';

        $this->load->view('global/header', $this->data);
        $this->load->view('global/header_buttons');
        $this->load->view('gov_table/philhealth_contribution');
        $this->load->view('global/footer');
    }
    public function getPhilhealthContribution(){
        $select_qry = $this->philhealth_model->get_all_philhealth_contribution();
        ...
        if(!empty($select_qry)){
            foreach ($select_qry as $value) {
                ...
                $finalData .= "<tr id='".$value->philhealth_contrib_id."'>";
